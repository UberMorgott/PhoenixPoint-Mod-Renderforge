...
struct Device12 : IDevice
{
    ...
    int Init(void* nativeResource, const wchar_t* inDllDir, const wchar_t* logDir) override
    {
        ...
        // DEV ONLY (RfFakeInitCode, Device.h): =2 does not CALL NVSDK_NGX_D3D12_Init_with_ProjectID at all, it
        ...
        // call NVSDK_NGX_D3D12_Shutdown1 on a device that never initialised NGX).
        ...
        // answer DLSS_OK_POST_ONLY. ngxInitialized stays 0, so Shutdown() skips NVSDK_NGX_D3D12_Shutdown1
        // and still releases this reference (plus ring/sharpen attached above) at Device12.cpp Shutdown().
    ...
    void Evaluate(const FrameParams& fp, bool passthrough) override
    {
        ...
        if (passthrough) {
            bool wantPost = fp.sharpness > 0.0f || PostShaderEnabled(fp.lutPreset, fp.lutStrength, fp.style, fp.colorVision, fp.grade);
            if (!wantPost || !sharpen.RunPassthrough(cl, color, output, owned, ring, srgbViews,
                                                      fp.sharpness, fp.lutPreset, fp.lutStrength, ring.ringIdx, fp.style, fp.colorVision, fp.grade))
            ...
            bool grade = PostShaderEnabled(fp.lutPreset, fp.lutStrength, fp.style, fp.colorVision, fp.grade);
            ...
            else if (doPost) sharpen.Run(cl, owned.out, fp.sharpness, fp.lutPreset, fp.lutStrength, ring.ringIdx, fp.style, fp.colorVision, fp.grade);
            ring.Stamp(2);
