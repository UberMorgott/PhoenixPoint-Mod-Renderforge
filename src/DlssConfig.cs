...
namespace Renderforge
{
    ...
    public class DlssConfig : ModConfig
    {
        ...
        [ConfigField("DirectX 12 direct inputs", "Developer: the upscaler reads the render targets in place. Off = copies them first (slower, the 2026-09-02 contract).")]
        public bool D3D12DirectInputs = true;
        ...
        private static readonly Dictionary<string, string[]> Ru = new Dictionary<string, string[]>
        {
            ...
            { nameof(D3D12DirectInputs), new[] { "DirectX 12: прямые входы", "Для разработчика: апскейлер читает рендер-таргеты напрямую. Выкл = сначала копирует их (медленнее)." } },
        };
