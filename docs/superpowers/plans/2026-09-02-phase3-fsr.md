...
- [x] **Step 1: Write `native\D3D12Ring.h`**
...
- [x] **Step 2: Include the ring in `native\Device12.cpp`**
...
- [x] **Step 3: Replace `Device12`'s ring members with the shared ring**
...
- [x] **Step 4: Point `Device12`'s call sites at the ring**
...
- [x] **Step 5: Add the header to CMake**
...
- [x] **Step 6: Build and run both probes — output must be unchanged**
...
- [x] **Step 1: Add the provider ids, error codes and exports to `native\RenderforgeNative.h`**
...
- [x] **Step 2: Widen `CreateParams` and `FrameParams` in `native\Device.h`**
...
- [x] **Step 3: Implement the new exports in `native\RenderforgeNative.cpp`**
...
- [x] **Step 4: Give `IDevice` the version hook**
...
- [x] **Step 5: Build (nothing implements FSR yet — the linker needs a stub)**
...
- [x] **Step 1: Write `native\FfxLoader.h`**
...
- [x] **Step 2: Write `native\FfxLoader.cpp`**
...
- [x] **Step 3: Point CMake at the FidelityFX SDK headers and add the source**
...
- [x] **Step 1: Write `native\Fsr12.cpp`**
...
- [x] **Step 1: Add the FSR run to `native\probe\dlss_probe.cpp`**
...
- [x] **Step 2: Factor the D3D12 bring-up out of `RunD3D12` so `RunFsr` can reuse it**
...
- [x] **Step 3: Add the `--fsr` flag to `wmain`**
...
- [x] **Step 4: Build and run it by hand (build-native.ps1 wires it in at Task 6)**
...
- [x] **Step 1: Write `LICENSE-AMD.txt`**
...
- [x] **Step 2: Add the AMD signature check and staging to `build-native.ps1`**
...
> **Deviations (Tasks 7/8, as shipped):**
> - Auto order on D3D12 is `NVIDIA -> DLSS, else FSR, else Off` — the plan's `else XeSS` step is deliberately
>   skipped until Phase 4 lands (`src/Upscaler.cs` `Resolve`). Intel GPUs therefore land on FSR for now;
>   XeSS is slotted in between FSR and Off once its provider exists.
> - `Resolve(Auto)` returns `Upscalers.Running` once the shim is up, so Auto never disagrees with the latched
...
- [x] **Step 1: Add the new imports to `src\Native.cs`**
...
- [x] **Step 2: Write `src\Upscaler.cs`**
...
- [x] **Step 3: Add the config field to `src\DlssConfig.cs`**
...
- [x] **Step 4: Make `Feature.Fsr` a real check in `src\Availability.cs`**
...
- [x] **Step 5: Select the provider before init in `src\RenderforgeMod.cs`**
...
- [x] **Step 6: Build the managed assembly**
...
- [x] **Step 1: Wire the UPSCALER row to `Cfg.Upscaler` in `src\Pickers.cs`**
...
- [x] **Step 2: Make the quality row provider-aware in `src\GraphicsPanel.cs`**
...
- [x] **Step 3: Feed the camera to the shim in `src\DlssDriver.cs`**
...
- [x] **Step 4: Report the live provider in `src\Overlay.cs`**
...
- [x] **Step 1: Add the AMD binaries and licence to the deploy set**
...
- [x] **Step 2: Deploy to the automation install**
...
> **2026-09-02 (owned-resource build `54af332`):** Steps 1, 2, 3, 6, 7 PASS on the 3.1.5 fallback path — profile `ModConfig.json` `Upscaler=3`, launched `-mods -force-d3d12 -force-d3d12-debug`, Quality tactical (853x480 → 1280x720) 300 s hold + 1 `start-mission` load, process alive 352 s, `GetStatus` `gen=Live q=1 lastError=0`, shot `own-fsr.png` (session scratchpad) — lit cave, fog of war, no smearing. Provider evidence: `%TEMP%\renderforge-d3d12.log` has no NGX/XeSS `Attach`/`Create` lines and the first ring submission already declares 4 states (ffx dispatches without a create list). Caveat: `Fsr12::Init` does not call `RfDbg::Attach`, so the in-game debug-layer count for FSR is unmeasured (the `dlss_probe --fsr` run with the debug layer reports 0 state mismatches on the same `OwnedSet12` path). `Player.log` is shared with a parallel `D:\PP-Instance3` run, so its provider line was not usable as evidence. Native AA / overlay shots (Steps 4, 5) not taken this pass.
>
> **2026-09-02 jitter A/B (build `fd394fe`):** 4 sign combos (`-1,1 / 1,1 / -1,-1 / 1,-1`) at Ultra Performance (426x240 → 1280x720), ALN_PLT_Nest_48x48_A seed 12345 — all indistinguishable on stills. Defaults `(-1,+1)` kept; shots `docs\shots\jitter-ab\fsr-*.png`. Still-frame limitation: same scene+seed lands different camera/unit each launch.

- [x] **Step 1: Force FSR in the config and launch under D3D12**
...
- [x] **Step 2: Wait for the bridge, then start a mission**
...
- [x] **Step 6: Log must be free of FFX and provider errors**
...
- [x] **Step 7: Stability gate — three mission loads**
...
- [x] **Step 2: Write `docs\research\fsr-ffx-api-d3d12-contract.md`**
...
- [x] **Step 3: Add the note to the research index**
