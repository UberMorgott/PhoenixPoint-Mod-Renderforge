...
> **2026-09-02 (later) — UNBLOCKED by Task 2b's child HWND: real XeSS-FG provider, PASS on `D:\PP-Instance3` (profile `...593`, `-mods -force-d3d12 -force-d3d12-debug`, `RENDERFORGE_D3D12_DEBUG=1`, `ALN_PLT_Nest_48x48_A` seed 12345, 1280x720, ModConfig `Upscaler:4`/`Renderer:2` latched so the XeSS upscaler runs; RTX 5070 Ti = cross-vendor DP4a path).** `native\FgXess.cpp` is now a full `IFgProvider` (`Name()="xess"`). Create, on the main thread: pin `<modDir>\libxell.dll` + `libxess_fg.dll` with `LoadLibraryW` (both import libs linked `/DELAYLOAD`, `CMakeLists.txt`, the `Xess12.cpp` shape) → `xefgSwapChainGetVersion`/`xellGetVersion` logged (`xess: XeSS-FG 1.3.1, XeLL 1.3.2`) → `xellD3D12CreateContext` (`xell_d3d12.h:28`) + `xellSetLoggingCallback` → `xefgSwapChainD3D12CreateContext` (`xefg_swapchain_d3d12.h:120`) + `xefgSwapChainSetLoggingCallback` → `xefgSwapChainSetLatencyReduction(fg, xell)` (`xefg_swapchain.h:431`, guide `:218-231` mandatory on every GPU, before init) → `xefgSwapChainGetProperties` (`:351`, pre-init reports only `maxSupportedInterpolations`, `:339-340`) = **1** → `caps 0x1`, 3x/4x refused with `FG_ERR_UNSUPPORTED_MULTIPLIER` before any chain (cached per process so the driver's 4x/s retry costs no context) → `xefgSwapChainD3D12InitFromSwapChainDesc(fg, child, &desc, NULL, queue, factory, &ip)` (`:216`, guide `:288-301`: `pApplicationSwapChain=NULL`, the child from `FgHostChildHwnd(s)` has no other chain) with `initFlags=INVERTED_DEPTH`, `maxInterpolatedFrames=multiplier-1`, `uiMode=XEFG_SWAPCHAIN_UI_MODE_BACKBUFFER_HUDLESS` (guide `:602-605`, `:623`: hud-less required, UI texture not used), desc = the host's (`FLIP_DISCARD`, R8G8B8A8_UNORM, 3 buffers, flags 0, `STRETCH`) → `xefgSwapChainD3D12GetSwapChainPtr` (`:232`) = the proxy the host presents (`chain=child`) → `xefgSwapChainSetUiCompositionState(ENABLED)` (`:511`, guide `:531-535`) → `xellSetSleepMode{bLowLatencyMode=1}` (`xell.h:161`, guide `:237-238` XeLL enabled before FG). Per frame: `Prepare` (render thread, `DLSS_EV_FG_PREPARE`) bumps the provider's OWN `presentId` (+1 per Prepare — XeLL logged `Sleep() called for frame N but expected N-1` 286x in the first run because the host's single frame slot skips/duplicates `FgFrame.frameId` when Unity's main thread runs ahead; the owned twins are always the latest frame, only the constants can be one frame stale), `xellSleep` (`xell.h:177`, first call for a new id, xell guide `:297`), markers `SIMULATION_START/END`, `RENDERSUBMIT_START` (`xell.h:74-79`, all six required), `xefgSwapChainD3D12TagFrameResource` (`:255`) x3 on the owned twins with `XEFG_SWAPCHAIN_RV_UNTIL_NEXT_PRESENT`, `incomingState=COMMON`, `pCmdList=NULL` (guide `:703-704`, `:763-764`): MV + depth at `o->w x o->h` (guide `:798` same size), hud-less = owned `out` at back-buffer size/format (guide `:571-572`, `:860`), then `xefgSwapChainTagFrameConstants` (`:365`; row-major view/proj, jitter `(-jx, +jy)` = `Xess12.cpp` signs, MV scale straight through, `resetHistory`, `frameRenderTime=dtMs`). `Generate` (Present hook, before the host's copy + `proxy->Present`): `xefgSwapChainSetPresentId(presentId)` (`:390`, guide `:1020-1022`) + `RENDERSUBMIT_END`, `PRESENT_START`; new `IFgProvider::AfterPresent(hr)` (default no-op, `Fg.h`; called right after the shadow Present in `FgHostOnPresent`): `PRESENT_END` + `xefgSwapChainGetLastPresentStatus` (`:401`) → `FgPresentedAdd(framesPresented-1)` (the host counts the real one; never faked). `SetEnabled` → `xefgSwapChainSetEnabled` (`:377`). Destroy (caller thread): `FgHost.DestroyChain` now releases the host's shadow reference BEFORE `prov->Destroy()` (`xefgSwapChainDestroy` refuses with an outstanding proxy ref, guide `:1076-1077`; FSR's swapchain context and the DComp visual hold their own refs, so both other chains are unaffected), then `xefgSwapChainDestroy` (`:421`), then `xellDestroyContext` (`xell.h:153`, guide `:1080`). Modules stay resident.
> Host/managed: `FgHostInit` keeps `H.lastCaps` from the provider even when Create is refused and sets `reason=multiplier above what this provider supports on this GPU` (or `vendor DLLs missing from the mod folder`) into `Fg_Status`; `Fg_Caps` returns it while no chain is up, `FrameGen.Caps` no longer gates on `live`, and `Pickers.FrameGenReason` lost the XeSS block, so 3x/4x grey with `Not supported by this GPU` from caps. `build-native.ps1` verifies + stages `libxell.dll 1.3.2.10` (Intel-signed) next to `libxess_fg.dll 1.3.1.78`; `deploy.ps1` copies it. Build: 4x `PROBE OK`, `build-native: OK`, 0 warnings; `dotnet build -c Release` 0/0.
> Measured (run 2, after the present-id fix): `SetFrameGen ["X2"]` → +5 s `fg=live provider=xess enabled=1 multiplier=2 chain=child child=0000000000BE0014 childHr=0x00000000 hit=-1 focus=game fg=0 shadow=00000202EF6B0B80 out=1280x720 flags=0x0 caps=0x1 lastError=0 presentHr=0x00000000 presented=9796 fps=114 frameId=864`; +10 s `… presented=10360 fps=111 frameId=1146` (564 presented / 282 rendered = 2.00x). Presented deltas over 5 x ~1.04 s: **+112/+110/+118/+126/+128** (rendered +56/+56/+58/+63/+64, ratio 1.96-2.03). +60 s alive `presented=15768 fps=112 frameId=3850`. `SetFrameGen ["X3"]` → `fg=off … multiplier=3 … caps=0x1 lastError=6 … reason=multiplier above what this provider supports on this GPU`, process alive, `xess: 3x asked, 2x supported` once. Run 1 (same build minus the present-id fix) also covered Off → alive 10 s → `X2` live again on a new child, `MoveWindow` +100x60 → `host: teardown (resize 1430x810)` → live `out=1430x810 … fps=84` → back → live `out=1280x720 … fps=95`; every teardown `xess: destroyed (XeSS-FG 1.3.1, generated N, lastRc 0)` (4597/317/284/307), `host: teardown … provider 47-63 ms`. Desktop `docs\shots\fg-xess-desktop.png` (PrintWindow, 853x480): live scene + HUD, overlay `Upscaler: XeSS 2.0.2 DP4a`, `FG: XeSS 2x`, `FPS: 59 / 118 (17,0 ms)` (the TFTV dialog is TFTV's own, from the `start-mission` plan). `renderforge_fg.log` run 2: 0 `Sleep()` mismatches, one `xess: frameGenResult 3` (WARNING_TOO_FEW_FRAMES on the first present), no tag/constants errors. Debug layer (`%TEMP%\renderforge-d3d12.log`): run 1 3848 lines / run 2 2031 lines, **0 `REMOVED`, 0 `CORRUPTION`, 0 errors naming `Renderforge`/XeSS/XeFG/Intel/proxy**; the rest is Unity's `Unnamed` id=527 / id=538 / id=1315 (present with FG off too). Rendered fps under XeSS-FG (~56-64) is below FSR-FG's (~65-70) on this GPU: DP4a interpolation + `xellSleep` inside the render event; not tuned. ModConfig restored (`FrameGen:0`, `Upscaler:1`, `Renderer:0`), Instance3 stopped.
...
> **2026-09-02 result on `D:\PP-Instance3` (profile `...593`, `-mods -force-d3d12 -force-d3d12-debug`, `RENDERFORGE_D3D12_DEBUG=1`, `ALN_PLT_Nest_48x48_A` seed 12345, 1280x720, ModConfig `Upscaler:2`/`Renderer:2`, RTX 5070 Ti, driver 596.49): PARTIAL — the DLSS-G proxy chain comes up on the child HWND and presents stably through every gate, but DLSS-G never emits a generated frame (ratio 1.00 at 2x/3x/4x). Shipped as `native\FgStreamline.cpp` (`MakeFgProviderStreamline`, `Name()="dlss"`), Streamline 2.12.0 manual hooking.**
> Sequence (all verified against the SDK): `LoadLibraryW(<modDir>\sl.interposer.dll)` + `GetProcAddress` for every entry point (ManualHooking.md:19, :179-187) → `slInit` once per process with `eUseManualHooking | eUseDXGIFactoryProxy | eDisableCLStateTracking | eUseFrameBasedResourceTagging`, features `{kFeatureDLSS_G, kFeatureReflex, kFeaturePCL}`, `pathsToPlugins`/`pathToLogsAndData` = mod dir (`sl.log` lands there), `engine=eUnity`, `projectId=kProjectId` (ManualHooking.md:90-107, sl_core_types.h:509-542, :549-588) → `slIsFeatureSupported` x3 with the device LUID (DLSS_G.md:126-165) → `slSetD3DDevice(native)` (ManualHooking.md:409-423) → `slGetFeatureFunction` for `slDLSSGSetOptions/GetState`, `slReflexSetOptions/Sleep`, `slPCLSetMarker` (sl_core_api.h:291-303, after the device) → `slGetFeatureVersion` = `SL 2.12.0 NGX 310.7.129` → `slReflexSetOptions{eLowLatency}` (Reflex.md:153-177) → `slUpgradeInterface(&device)` once (ManualHooking.md:195-203). Per chain: `proxyDevice->CreateCommandQueue(DIRECT)` (sl_hooks.h:61-62, DLSS-G names it `nv.sl.dlss_g.cmdQ.game`) → `slUpgradeInterface(&factory)` → `proxyFactory->CreateSwapChainForHwnd(queue, child, FLIP_DISCARD x3, flags WAITABLE|ALLOW_TEARING)` (ManualHooking.md:205-215; **without the flags the first proxy Present fails `0x887A0001`** — DLSS-G calls `SetMaximumFrameLatency` on the real chain, sl.log `swapChainData.cpp:471`) → QI `IDXGISwapChain4` = the shadow chain the host presents → `slDLSSGGetState` → `numFramesToGenerateMax=5` → `caps 0x7`. Per frame: `Prepare` (DLSS_EV_FG_PREPARE, render thread) `slGetNewFrameToken(&idx)` (idx = own process-wide counter: SL errors `Setting different 'common' constants multiple times within the same frame` when an index repeats after a rebuild) → `slReflexSleep` → PCL `SIMULATION_START/END`, `RENDERSUBMIT_START` (sl_pcl.h:60-67) → `slSetConstants` (Unity view/proj transposed to SL's row-vector layout, `clipToPrevClip` from the previous frame's pair, jitter/mvecScale in NGX convention, `depthInverted`, `cameraMotionIncluded`) → `slSetTagForFrame` depth/mv/hudless from the owned twins, state COMMON, `eOnlyValidNow` on the prep list; `Generate` (Present hook) applies `slDLSSGSetOptions{eOn, numFramesToGenerate=mult-1, eRetainResourcesWhenOff}` on the presenting thread once, then `RENDERSUBMIT_END`, `PRESENT_START`; `AfterPresent`: `PRESENT_END`, `slDLSSGGetState` → `numFramesActuallyPresented-1` → `FgPresentedAdd`. Host (`FgHost.cpp`): new `IFgProvider::PresentQueue()`; the back-buffer copy runs on that queue after `Chain::xfence` (Unity queue → provider queue) and Unity's queue then waits on the ring fence (**one shared fence signalled from two queues retired out of order → allocator reset in flight, debug layer id=541 → `DXGI_ERROR_INVALID_CALL` device removal; fixed with the second fence**). `SetEnabled(false)` applies `eOff` immediately (DLSS_G.md:745-748: off before the chain is released), `Destroy` releases queue + factory proxy; Streamline stays initialised for the process (deviation: no `slShutdown` per rebuild — `slInit` loads plugins + NGX every time).
> DLLs staged by `build-native.ps1` + `deploy.ps1` (Authenticode NVIDIA, exact versions): `sl.interposer.dll`, `sl.common.dll`, `sl.dlss_g.dll`, `sl.reflex.dll`, `sl.pcl.dll` = 2.12.0.0 (`refs\Streamline\bin\x64`), `nvngx_dlssg.dll` = 310.7.129.0 (`refs\Streamline\latest-dll`, asserted; the SDK's own is 310.7.0). NGX coexistence: no conflict — SL's own NGX (`app id 100721531`, same project id/engine as `Device12.cpp`) initialised next to the shim's `NVSDK_NGX_D3D12_Init_with_ProjectID`, DLSS SR kept running (`create=NVSDK_NGX_Result_Success`). Side effects seen in sl.log: SL/NGX use `C:\Temp\` (`%TEMP%`) as "application data path" and the driver launches `nvngx_update.exe -bootstrap` even with OTA flags off.
> Measured (final build, production plugins): `SetFrameGen ["X2"]` → +10 s `fg=live provider=dlss enabled=1 multiplier=2 chain=child child=00000000001A0548 childHr=0x00000000 hit=-1 focus=none fg=0 shadow=0000027CC72275B0 out=1280x720 flags=0x0 caps=0x7 lastError=0 presentHr=0x00000000 presented=543 fps=58 frameId=1216`, +60 s `… presented=3104 fps=48 frameId=3698`; deltas over 1 s: **+61/61, +63/63, +58/58 (ratio 1.00)**. `X3` → `fg=live … multiplier=3 … caps=0x7 lastError=0 presentHr=0x00000000 presented=3591 fps=59 frameId=4185`, +59/59, +55/55; `X4` → `fg=live … multiplier=4 … presented=4196 fps=59 frameId=4790`, +62/62, +60/60. `Off` → `fg=off provider=- … caps=0x7 lastError=0` alive; `X2` again live on a new child; `MoveWindow` +100x60 → `host: teardown (resize 1430x810) … provider 46 ms` → live `out=1430x810`, back → live `out=1280x720`. Desktop `docs\shots\fg-dlssg-desktop.png` (PrintWindow): live scene + HUD, overlay `FG: DLSS 2x`, `FPS: 48 / 48`. Debug layer (`%TEMP%\renderforge-d3d12.log`, 5022 lines): **0 `REMOVED`, 0 `CORRUPTION`**, 8 id=538/527 on Unity's back buffers around the cross-queue copy, **2069 id=1047 `nv.sl.dlss_g.tex2d.fake-swapchain-buffer` still referenced by `nv.sl.dlss_g.cmdQ.pacer` when our queue writes it** (DLSS-G's pacer reads the fake buffer we overwrite next frame — the host writes `GetBuffer(GetCurrentBackBufferIndex())` right after Present without a wait DLSS-G expects; candidate root cause, not fixed). sl.log: only `Repeated slDLSSGSetOptions() call for the frame N` per rebuild + two benign warnings; `DLSSGState.status == eOk` throughout, `numFramesActuallyPresented == 1` every frame, and the real child chain's `GetFrameStatistics().PresentCount` == our presents (`sl: frame 600 … dxgiPresentCount=599`), i.e. **no generated frame ever reaches DXGI**.
> Tried without effect (one game session each, all ratio 1.00, no new sl.log line even at `logLevel eVerbose` with the DEVELOPMENT plugins + `sl.dlss_g.json showDebugText`): tag lifecycle `eValidUntilPresent`; tags depth+mv only; depth+mv+hudless+zero `kBufferTypeUIColorAndAlpha` (sl.common registers all four as "required" for DLSS-G at startup); legacy `slSetTag` without frame-based tagging; SL-internal frame indices (`slGetNewFrameToken(nullptr)`); `EngineType::eCustom` (Unity mode makes sl.reflex "use render submit markers to detect current frame"); `slDLSSGSetOptions` every frame; no `eRetainResourcesWhenOff`; `SetForegroundWindow(child)` (fails: a WS_CHILD cannot be foreground — if DLSS-G gates on the swapchain window being the foreground window, the child-HWND design itself is the blocker). Next candidates: wait on the chain's frame-latency waitable object / DLSS-G's pacer before the host's copy (the id=1047 race), or a top-level (owned, not child) FG window so the chain's HWND can be the foreground window.
> ModConfig restored (`FrameGen:0`, `Upscaler:1`, `Renderer:0`), Instance3 stopped, `D:\PP-Instance3\sl.*.json` removed.

> **2026-09-02 round 2 (RCA + fixes, `D:\PP-Instance3`, same scene/seed/flags, RTX 5070 Ti 596.49): PASS — DLSS-G 2x/3x/4x generate exactly.**
> **Root cause of ratio 1.00 = DLSS-G's FOCUS GATE, not the integration.** `sl.extra` `extra.cpp:97-124` `hasFocus()` compares `GetForegroundWindow()`'s PID with the process (or parent) PID; the plugin presents the real frame only while that holds, with **no log line at any level** (status stays `eOk`, `numFramesActuallyPresented=1`). Every unattended run launches the game from a terminal, so the game window is never the foreground window (`Fg_Status … fg=0`). Proof: development plugins (`refs\Streamline\bin\x64\development`) + `D:\PP-Instance3\Mods\Renderforge\sl.dlss_g.json` `{"runWhenNoFocus":true}` (`dlfgConfig.cpp:85 Read 'runWhenNoFocus:1' from sl.dlss_g.json`, dev-only: `sl.plugin\plugin.cpp:296-313`) → `dlfgPresent.cpp:1030[presentCommon] DLSS-G interpolation state changed from disabled to enabled (mode=sl::DLSSGMode::eOn, numFramesToGenerate=1, SyncInterval=0)` and ratio 2.00/3.00/4.00 on the UNCHANGED round-1 code; the same session with production plugins, window not foreground → 1.00 (`+58/58, +60/59, +58/58`). The verbose log (`sl.interposer.json` `{"logLevel":2,…}` beside the exe, 1494 lines) contains no skip/drop reason — the grep for `shouldInterpolate|presentCommon|processFrame|presentFrame|dlfgPresent.cpp|pacer.cpp|viewportManager.cpp|focus|skip|drop` yields only the state-change lines above, `dlfgPresent.cpp:800[presentCommon] Present() is called on the thread N`, and `dlfgPresent.cpp:410[sanitizeResourceExtent] Invalid backbuffer resource extent … Resetting extent to full backbuffer resource size (1280 x 720)` (benign). Production + focused window could not be measured this session (the user's own game held the foreground; stealing focus was out of bounds) — that is the one cell left for a hands-on check; a player's window is foreground by definition.
> **Fixes shipped** (Codex review `C:\Temp\cx\a44b7875465641d282f4b248d0753949.out.md` #2-#4): (1) marker order — `IFgProvider::BeforePresent()` (`Fg.h`) runs in `FgHostOnPresent` AFTER the copy into the proxy's current back buffer is submitted and right BEFORE `Present`; `FgStreamline.cpp` issues `RENDERSUBMIT_END`/`PRESENT_START` there (was in `Generate`, before the copy), `PRESENT_END` in `AfterPresent`; (2) token one-to-one — `Prepare` pushes its token into a 4-deep FIFO (`fifo/head/tail`), `BeforePresent` pops exactly one per proxy Present, a full ring skips the Prepare instead of overwriting a waiting token (never triggered: 0 `Present without a prepared frame` / `skipping this one` lines); (3) the id=1047 hazard — measured with the debug layer on and generation active (dev plugins): `sync=none` **35-48/s** id=1047 (`nv.sl.dlss_g.tex2d.fake-swapchain-buffer` referenced by `cmdQ.dlssg` reads while `cmdQ.game` writes, and vice versa — 2619 + 1576 in one run), CPU-waiting the copy fence before Present → **0/s** at 2x/3x/4x with fps unchanged (114 vs 98-112 at 2x, 212 at 4x); waiting the chain's frame-latency object instead made it worse (29/s). Shipped: `FgHost.cpp` waits `copy.fence` on the CPU before `Present` whenever the provider presents on its own queue (DLSS-G only; FSR/XeSS unchanged). Whole final run: **0 id=1047, 0 REMOVED/CORRUPTION, 0 debug-layer errors on our lists/resources** (415 errors total = the pre-existing id=527/538 on Unity's own RTs).
> **Measured (final build, dev plugins + runWhenNoFocus, 1280x720):** `SetFrameGen ["X2"]` → `fg=live provider=dlss enabled=1 multiplier=2 chain=child child=000000000176068A childHr=0x00000000 hit=-1 focus=game fg=0 shadow=000002C706B50C20 out=1280x720 flags=0x0 caps=0x7 lastError=0 presentHr=0x00000000 presented=491 fps=121 frameId=272`, +5 s `… presented=1091 fps=112 frameId=572` (600 presented / 300 rendered); 1 s deltas `+110/55, +106/53, +114/57, +116/58, +114/57` (**ratio 2.00**); `X3` → `… multiplier=3 … presented=3447 fps=174 frameId=1463`, `+159/53, +153/51, +144/48, +147/49, +162/54` (**3.00**); `X4` → `… multiplier=4 … presented=5713 fps=227 frameId=2033`, `+204/51, +208/52, +208/52, +204/51, +212/53` (**4.00**). `numFramesActuallyPresented` per frame = 2 / 3 / 4 (`sl: frame 3 (token 852): presented=3`, `(token 1433): presented=4`); real child chain `GetFrameStatistics().PresentCount` = 2x our presents (`frame 300 … dxgiPresentCount=594`, `frame 600 … dxgiPresentCount=1195`). `Off` → `fg=off … multiplier=4` alive; `X2` again 2.00; `MoveWindow` +100x60 → teardown/rebuild → `out=1430x810` 2.00; back → `out=1280x720` 2.00. Desktop `docs\shots\fg-dlssg2-desktop.png` (PrintWindow): overlay `FG: DLSS 2x`, `FPS: 64 / 130 (15.7 ms)` (dev-SDK watermark expected). Still seen once per rebuild: `defines.h:318[set] Repeated slDLSSGSetOptions() call for the frame N` (eOff at teardown and eOn of the new chain land in the same plugin frame; harmless).
> ModConfig restored (`FrameGen:0`, `Upscaler:1`, `Renderer:0`), production `sl.*`/`nvngx_dlssg.dll` re-deployed, `sl.interposer.json`/`sl.dlss_g.json` removed, Instance3 stopped.

- [ ] **Step 1: Write `native\FgStreamline.cpp`** (superseded by the shipped file — see the result note above)
...
> **2026-09-02 user crash, main menu DLSS-G (real install, 2560x1440 `FullScreenWindow`, vsync off, production SL plugins, window focused): Options → Graphics → DX12 + DLAA + FG 2x → window minimized, later `Crash!!!` in `RenderforgeNative.dll`.** Evidence in the user's logs: managed `FG live: … presented=52 fps=0 frameId=0` and nothing after; fg log `Present without a prepared frame - no markers` on EVERY present (514 KB); sl.log `Pacer flush has timed out on frame 2..8` → `interpolation state changed from enabled to disabled` → `Couldn't lock the mutex on sync present` → `[error] eDLSSGStatusFailReflexNotDetectedAtRuntime … 1 != 232`. `frameId=0` forever = `Fg_SetFrame`/`DLSS_EV_FG_PREPARE` never ran (the camera path — `PostProcessLayer.OnPreCull` postfix — does not run on that screen) while the chain was live, so every hooked Present drove the SL proxy Present with no frame token, no constants, no PCL markers: that is what wedges DLSS-G's pacer.
>
> Repro on `D:\PP-Instance3` (profile `...593` set to `FullScreenWindow` 2560x1440, ModConfig `Renderer:2 Mode:2(DLAA) Upscaler:2`, SL DEVELOPMENT plugins + `sl.dlss_g.json {"runWhenNoFocus":true}`, `-mods -force-d3d12`, HEAD `65e57c1` + hook depth log): in the HomeScreen the camera renders, so `SetFrameGen X2` came up healthy (`fg=live provider=dlss … presented=794 fps=397 frameId=13174`, ratio 2.00, `sl: frame 1 (token 1): presented=2 … (hr 0x887A000B)` = the same first-frame line the user saw, benign). The no-frame state was then forced with the new test knob `FrameGen.HoldPrepare` (PPCLI `set` on the static): identical to the user's log (`Present without a prepared frame` every present). Window: the minimize is Unity's own on `WM_ACTIVATE 0` in `FullScreenWindow` — it happened with FG OFF too whenever the harness took the foreground, and with FG live `ShowWindow(SW_RESTORE)` + activation restored it (`WM_SIZE RESTORED 2560x1440`, FG kept presenting) — not caused by the child window / SL (H3 refuted). Hook re-entrancy IS real but bounded: `hook: re-entered Present depth 2 on chain <native child> tid <render thread>` — DLSS-G presents the real child chain from inside our shadow Present when it has no markers; `IsApp` already rejected it (H1 unbounded recursion not reproduced; the guard is now explicit). The crash itself did not reproduce on the dev plugin (60 s + 60 s idle in the menu, process alive).
>
> Fixes (`fix(fg)`): `FgHost.cpp` — a Present with no frame prepared since the last one is IDLE: no copy, no shadow Present, no vendor call, Unity's Present forwarded unchanged, child window hidden (`FgWndShow(false)`, Unity's chain shows through, no stale frame) and shown again on the next prepared frame; status gains `idle=<consecutive idle presents>`. `FgHook.cpp` — per-thread depth counter, depth > 1 = straight passthrough (logged 16x). `FgStreamline.cpp` — full-size `sl::Extent` on the three tags. `FgWnd.cpp` — `WM_SIZE MINIMIZED/RESTORED`, `WM_ACTIVATE`, `WM_ACTIVATEAPP` logged. Verified on Instance3 (dev plugin, focused): menu live `+164/82 ratio=2.00`; `HoldPrepare=true` 60 s → `idle=2498 fps=80` (= the FG-off menu rate, 80), `host: idle: no prepared frame - child hidden …`, 0 SL warnings/errors during idle; `HoldPrepare=false` → `host: frames resumed after 3595 idle presents - child shown`, ratio 2.00 again; `start-mission` → `fg=live` ratio 2.00 in the mission; Off; `Application.Quit` clean. Debug-layer session (`-force-d3d12-debug` + `RENDERFORGE_D3D12_DEBUG=1`, `TEMP` redirected because Instance2 shares `%TEMP%\renderforge-d3d12.log`): 374 `D3D12 ERROR` all Unity's own (`Unnamed` lists / `BackBuffer`), **0 naming ours**, 0 `REMOVED`/`CORRUPTION`, fg log 0 fail/error, sl.log 0 error/timeout/mutex. Still open: `sanitizeResourceExtent … (0 x 0)` is about the backbuffer extent SL derives itself (once per chain, benign); `Repeated slDLSSGSetOptions() call` once per chain rebuild (benign).

> **2026-09-03 user crash #2 (Steam install, PRODUCTION SL plugins, window focused so DLSS-G really interpolated, 2560x1440 borderless, build 20cd0e3+915e341): stack overflow 460 ms after `SetFrameGen X2`.** The user's `renderforge_fg.log` (3 KB, `D:\Steam\…\Mods\Renderforge`): `hook: installed (vtable 00007FF837072688, Present 00007FF836FA2CA0 …)` → app chain `0000025531C36FE0` latched → proxy chain up → `host: first shadow Present(0, 0x0) -> 0x00000000` → then only `hook: re-entered Present depth 2 … depth 17 on chain 0000025531C36FE0 tid 21556 - passthrough` (the APP chain, the render thread, strictly nested) → log ends; sl.log ends 2 ms after `DLSS-G interpolation state changed from disabled to enabled`. No dump (the Unity crash folders of that night are all Instance2/3 runs). **Proven from the log + source:** `00007FF836FA2CA0` = `dxgi.dll+0x2CA0`, `vtable` = `dxgi.dll+0xD2688` (this boot's dxgi base `0x7FF836FA0000`, same in every Instance3 run) — so the forward of Unity's own Present at depth 1 (`FgHook.cpp` `HookPresent1` → `g_origPresent1(self, 0, …)`, the immutable dxgi ENTRY captured at install) came BACK into the hook on the same chain, and the depth-2 "passthrough" (`Reentered` → `g_origPresent1` again, old `FgHook.cpp:169/181`) forwarded through the SAME entry again → unbounded. The old message printed "Present" for both hooks; the bounce lever run below shows Unity 2019.4 D3D12 presents through **`Present1`** (slot 22). Something that runs between dxgi's `Present1` entry and its body dispatches through the vtable slot we own; on THIS machine with a direct launch it is nobody (the `first re-entry` line names `sl.dlss_g.dll+0x475E4` presenting its own native child chain at depth 2, `HookPresent`/`HookPresent1` unpatched, `dxgi Present entry dxgi.dll+0x2CA0`/`Present1 entry dxgi.dll+0x3140` undetoured) and **the Steam overlay (`GameOverlayRenderer64.dll`) is the one party present in the user's Steam-launched process and absent from every Instance3 process (122 modules, `steam_api64.dll` loaded, overlay not)** — it hooks the function it reads from a new swapchain's vtable on every `CreateSwapChain*`, which after our patch is OUR hook; the loop needs exactly a hook whose stored original is our function again. The user's next run names it: the fg log now carries `hook: first re-entry (… = APP …): caller <module+off>; slot Present=… Present1=…; dxgi Present entry … -> detoured to <module+off> (patched since install); HookPresent … (patched since install)`.
> **Fix (`fix(fg)`, `native\FgHook.cpp`, `Fg.h`, `FgHost.cpp`):** (1) the four originals are captured ONCE (`PatchSlot` keeps the first value, `UnpatchSlots` no longer NULLs them) and every forward uses them — the vtable is never read for dispatch; (2) `Reentered(self, sync, flags, pp, via, caller)`: a re-entered APP-chain Present at depth 2 is presented ONCE through the OTHER dxgi entry (`Present1` → `Present`, `Present` → `Present1` with zeroed `DXGI_PRESENT_PARAMETERS`; a detour sits on one entry), at depth 3 it is not presented at all — the thread's last result is returned, `hook: app Present bounces back through BOTH dxgi entries - not presenting, FG detached` once, `FgHostTearDown("Present hook re-entered 3 deep")`; a foreign chain (the vendor pacer's real child chain) passes through up to depth 4; the depth check now precedes the TEST/DO_NOT_SEQUENCE passthrough; (3) `FgHookHide()`/`FgHookShow()` bracket the provider `Create` in `FgHostInit` so a `CreateSwapChain*` hooker sees dxgi in the slots, never us (`hook: slot N taken by <module> while hidden - re-patched over it` if someone moved in); (4) diagnostics: `hook: installed (vtable …) Present=dxgi.dll+0x2CA0 …` names module+offset and any `-> detoured to` jmp target (E9 / FF 25 / mov rax,imm64;jmp rax) of every original, `hook: first re-entry (…)` names the caller (`_ReturnAddress`), the current slot owners and whether dxgi's entries / our hooks were patched since install; `re-entered Present|Present1 depth N on chain P (app)`. Test lever `RENDERFORGE_FG_BOUNCE=1|2` (env, off by default): the depth-1 app forward goes through the vtable slot on purpose (`2` = the depth-2 fallback bounces too).
> **Verified on `D:\PP-Instance3` (production SL plugins as shipped, `-mods -force-d3d12`, `SetForegroundWindow` after `AllowSetForegroundWindow(ASFW_ANY)` + `keybd_event(VK_MENU)` + `AttachThreadInput` → `GetForegroundWindow()==hwnd`, `focus=game fg=1`; registry `Screenmanager Resolution 2560x1440 / Fullscreen mode 1` so the window starts 2560x1440 `FullScreenWindow` like the user's, restored after):** menu `SetFrameGen X2` → `fg=live provider=dlss … out=2560x1440 caps=0x7 lastError=0 fps=462`, 60 s alive, ratio **2.00** (`presented` 3462→27368 vs `frameId` 1800→13753); `start-mission` → `fg=live` ratio 2.00 at fps 241 for 60 s; Off/On ×3 → live, ratio 2, `lastError=0` each time; `Application.Quit` clean (`sl: destroyed … generated 13632`, teardown 31-47 ms); fg log 128 lines, `re-entered=16` all `depth 2` on the native child chain from `sl.dlss_g.dll+0x475E4`, **0 app re-entries, 0 bounces, 0 fail/error**. Lever `RENDERFORGE_FG_BOUNCE=1`: `hook: first re-entry (Present1, depth 2, chain … = APP, tid …): caller RenderforgeNative.dll+0xAE96 …`, `re-entered Present1 depth 2 … (app) … - other entry once` every frame, ratio still 2.00, Off/On ×3, clean quit. Lever `=2`: `re-entered Present depth 3 … (app) - not presenting` + `bounces back through BOTH dxgi entries - not presenting, FG detached` once, **no stack overflow**; Unity then dies on its own (`CreateCommittedResource … failed (887a0005)` = DEVICE_REMOVED, the chain nobody can present any more) — the only outcome when both entries are compromised. Debug layer (`-force-d3d12 -force-d3d12-debug` + `RENDERFORGE_D3D12_DEBUG=1`, `TEMP` redirected): 585 `D3D12 ERROR`, **0 naming ours**, 0 `REMOVED`, 0 `CORRUPTION`, FG ratio 2.00 in menu and mission under the layer. `hr 0x887A000B` on `sl: frame 1` is `GetFrameStatistics` = `DXGI_ERROR_NOT_CURRENTLY_AVAILABLE` before the first present completed; frames 2/3 read `hr 0x00000000` — no latency handshake needed. Seen in passing, NOT fixed here: on a live resolution change (`Screen.SetResolution(2560,1440)` / the FullScreenWindow resize on focus) DLSS SR reports `eval=0xBAD00002 NVSDK_NGX_Result_FAIL_PlatformError … DLSS off` (`Player.log` `[ERROR] … DLSS off: NGX evaluate failed`) and once the process crashed 20 s in (`Crash_2026-09-02_220020615`, UnityPlayer main thread, with the working tree's managed changes deployed) — pre-existing upscaler path, no FG involved.
