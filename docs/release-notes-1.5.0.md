...
- **Crisp icons** -- new toggle under Options > Screen (on by default). The interface is authored for 4K, so at 1440p every icon is drawn at 0.67x or smaller and the vanilla bilinear UI atlas snaps between mip levels; Renderforge switches every mipmapped interface texture on the HUD to trilinear filtering with 8x anisotropy. Sprites without mipmaps are left unchanged; originals are restored when the toggle is off or the mod is disabled.
- **Crisp fonts removed.** Measured at 1440p it had no effect on sharpness (glyphs are already rasterised 1:1 for the screen) and covered 13 of 34 labels, while causing the per-glyph drift some of you saw in 1.4.1. The setting is ignored; the drift is gone with the feature.
