...
- **Image sliders** add ReShade-style Exposure, Black point, White point, Brightness, Contrast, Clarity, Vibrance and Saturation in the same post pass, live in tactical missions and on the Geoscape, all at identity by default. Clarity is a luma unsharp mask against a 13-tap Poisson blur (local contrast on fine detail). Scene only; the HUD stays unchanged. Every tooltip states the default, and one **Reset image settings** button returns every Renderforge image setting to its default — Sharpness, LUT filter and strength, all eight sliders, scene style with its strength and pixel size, vignette, shadow resolution, anisotropic filtering and LOD detail (Renderer, Upscaler, Quality, Frame generation and Colour vision stay as chosen).
- **Pixel-perfect UI** (on by default since 1.6.1) snaps the interface to the pixel grid (`Canvas.pixelPerfect` on the root overlay canvases) and pins interface textures to full resolution (`mipMapBias = log2(UI scale)` on every sprite texture, including ones other mods create at runtime): measurably sharper text at 1440p and no 1-pixel edge lines on runtime-created mod icons (measured 138/255 → 0 at every fractional position), but animated panels may move in whole-pixel steps. Applies live, no restart.
...
| Reset image settings | — | Button: every Renderforge image setting back to its default — Sharpness, LUT filter and strength, the eight sliders above, scene style with its strength and pixel size, vignette, shadow resolution, anisotropic filtering and LOD detail. Renderer, Upscaler, Quality, Frame generation and Colour vision stay as chosen. |
...
| Pixel-perfect UI | On | `Canvas.pixelPerfect` on every root screen-space overlay canvas (nested canvases inherit) plus `mipMapBias = log2(UI scale)` on every sprite texture (−0.585 at 1440p under the game's 4K UI reference; textures created later by other mods are pinned as they are created). Snaps interface elements to the pixel grid and samples icons from mip 0: sharper text at non-native UI scales (measured +2–7% edge contrast at 1440p) and no 1-pixel edge line on runtime-created mod icons (unpadded atlas sub-rect 138 → 0, Repeat-wrap full-rect 89 → 0 at every fractional position); animated panels may move in whole-pixel steps. Live, no restart; camera/world canvases untouched. A saved Off stays Off. |
